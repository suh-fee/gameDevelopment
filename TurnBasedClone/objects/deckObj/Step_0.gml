...
switch(global.state) {
	...
	if (start){ // will only be ran once, used to set up the initial deck in the game
	    ...
	    if (cards_in_hand < 3) {
			...
			card.tarX = room_width / 4 + (cards_in_hand * 70)
			card.tarY = 3 * room_height / 4
			card.faceUp = true
			card.choseable = true
			temp++
			...
			card.tarX = room_width / 4 + ((cards_in_other) * 70 )
			card.tarY = room_height / 4
			temp++
		} else {
			start = false
		...
		cardOne.tarX = room_width / 4 + (replaceOne * 70) //puts the cards in the right places
		cardOne.tarY = 3 * room_height / 4
		cardOne.faceUp = true
		cardOne.choseable = true
	
		cardTwo.tarX = room_width / 4 + (replaceTwo * 70)
		cardTwo.tarY = room_height / 4
		audio_play_sound(cardSound, 1, 0)
		global.state = state.play
